...
public class JdbcTemplatePostRepository implements PostRepository{
    ...
    private final RowMapper<PostDto> postListRowMapper = (ResultSet rs, int rowNum) -> {
...
//                rs.getObject("created_at", LocalDateTime.class));
    ...
    private final RowMapper<PostDto> postDetailRowMapper = (ResultSet rs, int rowNum) -> {
        return PostDto.builder()
                .id(rs.getInt("id"))
                .author(rs.getString("author"))
                .title(rs.getString("title"))
    ...
    public List<PostDto> findAll() {
        ...
        return jdbcTemplate.query("SELECT id, author, title, secret, created_at FROM post2", postListRowMapper);
    }

    @Override
    public PostDto findById(int id) {
        // jdbcTemplate.queryForObject는 1번만 실행해서 객체를 반환
        return jdbcTemplate.queryForObject("SELECT id, author, title, content, secret, created_at FROM post2 WHERE id = ?", postDetailRowMapper, id);
    }
